...
| :--- | :--- | :--- |
| **Lienzo / Fondos destacados** | Verde menta tenue (Mint Whisper) | `#F4F8F5` |
| **Estructura y H2** | Verde salvia | `#76927A` |
| **Títulos principales y H1** | Verde Pino PSLL | `#113927` |
| **Texto de lectura** | Verde tinta (nunca negro puro) | `#2F3A30` |
| **Acento dinámico y alertas** | Coral Active Wave | `#E98F71` |
| **Acentos secundarios** | Melocotón | `#EDB090` |
| **Institucional UPO** | Azul Marino UPO | `#0B1C36` |
| **Retículas y bordes** | Gris pizarra | `#E2E8F0` |
| **Tipografía Titulares** | Outfit (alternativa: Inter / Segoe UI) | 24 pt (Título), 16 pt (H1), 13 pt (H2), 11 pt (H3) |
...
1. **Jerarquía Tipográfica y Organización Visual de Secciones:**
   ...
   - Aplicación estricta de `Heading 1` (15.5 pt en Verde Pino `#113927`), `Heading 2` (12.5 pt en Verde Salvia `#76927A`) y `Heading 3` (11 pt en Verde Pino `#113927`) alineados a la izquierda para navegación en Word.
...
3. **Cajas Destacadas (*Callout Boxes*):**
   - Tabla unificada de 1 celda con fondo temático, borde izquierdo grueso de 3 pt (26 dxa), padding interior calibrado y título en negrita con icono temático:
     ...
     - 💡 **Concepto Clave / Conceptos Clave del Tema (*Concept Box*):**
       ...
       - Fondo verde menta tenue (`#F4F8F5`), borde en Verde Salvia (`#76927A`), icono `💡` y título en Verde Pino (`#113927`). Los conceptos se listan de forma itemizada y jerárquica con viñeta salvia y término en negrita.
     - 📊 **Dato Empírico / Noticia (*Case Box*):** Fondo verde menta tenue (`#F4F8F5`), borde en Verde Pino (`#113927`). Aplicaciones reales de la EPA, SEPE o Banco de España.

4. **Tablas Estilizadas:**
   - Fila de cabecera con fondo Verde Pino (`#113927`) o Salvia (`#76927A`), texto en blanco y negrita.
   - Filas de datos con texto en Verde Tinta (`#2F3A30`), bordes horizontales limpios (`#E2E8F0`) y sombreado alterno sutil.
...
5. **Gestión Integral y Remasterización de Figuras:**
   ...
   - **Maquetación Reglamentaria:** Todas las figuras se insertan centradas (ancho estándar ~6.2 pulgadas), con pie descriptivo en 9 pt Outfit negrita Verde Pino (`#113927`) y fuente en 8 pt Calibri cursiva Verde Salvia (`#76927A`):  
...
6. **Cabecera Institucional Inicial (Página 1):**
   ...
   - Todos los párrafos de metadatos académicos formateados como líneas independientes alineadas a la izquierda (`WD_ALIGN_PARAGRAPH.LEFT`):
     - `UNIVERSIDAD PABLO DE OLAVIDE` (9 pt Outfit negrita en Verde Pino `#113927`).
   ...
   - Celda derecha con el logotipo oficial de la asignatura con los anillos de emparejamiento y la onda coral (`logo_psll_upo.png`).
   - Línea separadora horizontal (`―` * 48) alineada a la izquierda en Verde Salvia (`#76927A`).

7. **Encabezado y Pie de Página Corporativo:**
   - **Encabezado Justificado a la Izquierda:** Alineado a la izquierda (`WD_ALIGN_PARAGRAPH.LEFT`) con el título del tema en verde salvia (`#76927A`), 8 pt Outfit.
...
8. **Tratamiento Avanzado de Fórmulas Matemáticas (`m:oMath`):**
   ...
   - Las ecuaciones principales se maquetan automáticamente **centradas, con margen vertical holgado (6 pt antes / 6 pt después) y en negrita Verde Pino (`#113927`, 11 pt)**.
   - Las definiciones de parámetros y condiciones de desviación se integran en la jerarquía visual con viñetas institucionales y negritas en verde pino.
